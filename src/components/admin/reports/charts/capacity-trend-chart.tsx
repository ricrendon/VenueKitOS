...
interface CapacityTrendChartProps {
  data: { date: string; utilization: number }[];
}
...
export function CapacityTrendChart({ data }: CapacityTrendChartProps) {
  if (data.length === 0) {
    return (
      <Card>
        <CardContent>
          <h3 className="text-body-m font-medium text-ink mb-4">Capacity Utilization</h3>
            ...
            <AreaChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id="capGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#8EAA92" stopOpacity={0.3} />
                  <stop offset="95%" stopColor="#8EAA92" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              />
              <ReferenceLine y={80} stroke="#C96E4B" strokeDasharray="3 3" label={{ value: "80% target", fontSize: 10, fill: "#C96E4B" }} />
              <Area
                type="monotone"
                dataKey="utilization"
                stroke="#8EAA92"
