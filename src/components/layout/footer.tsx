...
const footerLinks = {
  visit: [
    { href: "/booking/open-play", label: "Open Play" },
    ...
    <footer className="bg-ink text-cream-200">
      <div className="container-content section-padding">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          {/* Visit */}
          <div>
            <h4 className="font-display font-semibold text-label text-cream-50 uppercase tracking-wider mb-4">
              Visit
            </h4>
            <ul className="space-y-3">
              {footerLinks.visit.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-body-s text-cream-300 hover:text-cream-50 transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
